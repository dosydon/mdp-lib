...
TRACKS := tracks/known/ring-3-error.track tracks/known/ring-4-error.track tracks/known/square-3-error.track tracks/roads-extreme.track
...
merged = $(foreach track,$(TRACKS),$(foreach method,$(METHODS),$(RESULT_DIR)/$(GIT_VERSION)/$(track)/$(method).json))
...
logs = $(foreach track,$(TRACKS),$(foreach method,$(METHODS),$(foreach maxtrial,$(MAX_TRIALS),$(RESULT_DIR)/$(GIT_VERSION)/$(track)/$(method)_$(maxtrial).log)))
jsons = $(foreach track,$(TRACKS),$(foreach method,$(METHODS),$(foreach maxtrial,$(MAX_TRIALS),$(RESULT_DIR)/$(GIT_VERSION)/$(track)/$(method)_$(maxtrial).json)))
...
.PHONY: run
run: $(logs) $(jsons) $(merged) $(pngs) $(comparisons)
